...
if (isset($_SESSION['lastAccessTime'])) {
    ...
    if ($currentTime - $lastAccessTime < $limit) {
        if($isChinese) exit('<i class="mdui-icon material-icons">warning</i> 访问频率过高，请等待2分钟后再试！');
        else exit('<i class="mdui-icon material-icons">warning</i> Request rate is too high！');
    }
}
...
if (empty($gain) || empty($title)) {
    if($isChinese) exit('<i class="mdui-icon material-icons">warning</i> 评论失败！表单中未包含昵称或留言内容！');
    else exit('<i class="mdui-icon material-icons">warning</i> Make sure your nickname and message has been filled in textarea！');
} else {
    ...
    if ($key) {
        if($isChinese) exit('<i class="mdui-icon material-icons">warning</i> 昵称或评论中存在不当词汇：' . $key);
        else  exit('<i class="mdui-icon material-icons">warning</i> Please speak cautiously：' . $key);
    } else {
        ...
        if ($conn->connect_error) {
            if($isChinese) die('<i class="mdui-icon material-icons">error</i> 连接失败: ' . $conn->connect_error);
            else die('<i class="mdui-icon material-icons">error</i> Connect ERR: ' . $conn->connect_error);
        }
        ...
        if ($conn->query($sql) === TRUE) {
            if($isChinese) echo '<i class="mdui-icon material-icons">info</i> 评论已提交到sql<br>';
            else echo '<i class="mdui-icon material-icons">info</i> Comment has been uploaded to sqldb<br>';
        } else {
        ...
        if($isChinese) exit('<i class="mdui-icon material-icons">done</i> 评论成功!!');
        else exit('<i class="mdui-icon material-icons">done</i> Comment uploaded!!');
    }
}
?>